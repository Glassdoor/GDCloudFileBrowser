...
@property (nonatomic, strong) DBPath *dropboxPath;
...
- (instancetype)initWithDropboxProvider:(BOOL)isRoot path:(DBPath *)path delegate:(id <GDFilePickerDelegate>)delegate {
    if (self = [super initWithNibName:NSStringFromClass([self class]) bundle:nil]) {
        ...
        if (isRoot) { // This viewController is used recursively. Add an observer only for the 'root folder'
            [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(didLinkDropboxSuccessfully:) name:kLMDropboxLinkSuccessfull object:nil];
        }
...
- (void)viewDidLoad {
    ...
    if (self.provider == GDCloudProviderDropbox) {
        if (![self.dropboxHelper dropboxAccountLinked]) { // no accounts are currently linked.
            ...
            if (self.isRoot) {
                // this notification will set up the account
...
- (void)dealloc {
    if (self.isRoot && (self.provider == GDCloudProviderDropbox)) {
        [[NSNotificationCenter defaultCenter] removeObserver:self name:kLMDropboxLinkSuccessfull object:nil];
    }
}
...
- (void)didLinkDropboxSuccessfully:(NSNotification *)notification {
    if ([DBFilesystem sharedFilesystem]) {
        [self didFinishLinkingAccount];
    } else if ([self dbAccount]) {
        DBFilesystem *filesystem = [[DBFilesystem alloc] initWithAccount:[self dbAccount]];
...
- (void)downloadGDriveFiles {
    ...
    [self.googleDriveHelper downloadGDriveFilesFrom:self.gDrivePath completion:^(BOOL success, NSError *error) {
